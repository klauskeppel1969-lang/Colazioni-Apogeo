...
// ── STAMPA: apre finestra separata, solo articoli con qty > 0 ─────────────

document.getElementById('btn-print').addEventListener('click', () => {
  const selectedItems = Object.values(state.items).filter(i => i.selected && i.qty > 0);
  if (selectedItems.length === 0) { alert('Nessun articolo da stampare (selezionare almeno un articolo con quantità > 0).'); return; }
  ...
  let rows = '';
  cats.forEach(cat => {
    const items = selectedItems.filter(i => i.category === cat).sort((a,b) => a.name.localeCompare(b.name));
    rows += `<tr><td colspan="3" style="background:#333;color:#fff;font-weight:bold;font-size:9pt;letter-spacing:0.08em;text-transform:uppercase;padding:4px 8px;">${CAT_EMOJI[cat] || ''} ${cat}</td></tr>`;
    items.forEach(item => {
      rows += `<tr>
        <td style="padding:5px 8px;border-bottom:1px solid #ddd;font-size:11pt;">${escHtml(item.name)}${item.isExtra ? ' <span style="font-size:8pt;color:#c8a45a;font-weight:bold;">[EXTRA]</span>' : ''}</td>
        <td style="padding:5px 8px;border-bottom:1px solid #ddd;font-weight:bold;font-size:12pt;text-align:center;">${item.qty}</td>
        <td style="padding:5px 8px;border-bottom:1px solid #ddd;font-size:9pt;color:#666;font-style:italic;">${escHtml(item.note || '')}</td>
      </tr>`;
    });
  });

  const html = `<!DOCTYPE html><html lang="it"><head><meta charset="UTF-8">
    <title>Ordine Colazioni</title>
    <style>
      body { font-family: Arial, sans-serif; margin: 20px; color: #111; }
      h2   { font-size:14pt; margin-bottom:2px; }
      p    { font-size:10pt; color:#666; margin-bottom:14px; }
      table { width:100%; border-collapse:collapse; }
      th   { border-bottom:2px solid #333; text-align:left; padding:5px 8px; font-size:10pt; background:#f0f0f0; }
      tr:nth-child(even) td { background:#f8f8f8; }
      .foot { font-size:8pt; color:#aaa; margin-top:14px; }
    </style>
  </head><body>
    <h2>☕ ORDINE COLAZIONE — Hotel Apogeo</h2>
    <p>${now} &nbsp;·&nbsp; ${selectedItems.length} articoli</p>
    <table>
      <thead><tr><th style="width:45%">Articolo</th><th style="width:10%;text-align:center;">Qtà</th><th>Note</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <p class="foot">Generato da Ordine Colazioni WebApp</p>
