...
// ── STAMPA: finestra separata, layout operativo, solo qty > 0 ─────────────
...
document.getElementById('btn-print').addEventListener('click', () => {
  ...
  cats.forEach(cat => {
    ...
    // Separatore categoria (riga leggera con nome categoria)
    rows += `
      <tr class="cat-sep">
        <td colspan="2">
          <span class="cat-label">${CAT_EMOJI[cat] || ''} ${cat.toUpperCase()}</span>
        </td>
      </tr>`;
    ...
    items.forEach(item => {
      rows += `
        <tr class="item-row">
          ...
          <td class="col-name">
            <span class="item-name">${escHtml(item.name)}${item.isExtra ? ' <span class="tag-extra">[EXTRA]</span>' : ''}</span>
...
<title>Ordine Colazioni — Hotel Apogeo</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* intestazione */
  .page-header { margin-bottom: 24px; border-bottom: 3px solid #111; padding-bottom: 12px; }
  .page-title  { font-size: 11pt; font-weight: normal; color: #666; letter-spacing: .06em; text-transform: uppercase; }
  .page-date   { font-size: 20pt; font-weight: 900; color: #111; margin-top: 4px; letter-spacing: .02em; }
  .page-meta   { font-size: 9pt; color: #888; margin-top: 6px; }

  /* tabella */
  table { width: 100%; border-collapse: collapse; }

  /* separatore categoria */
  .cat-sep td { padding: 14px 0 4px; }
  .cat-label  { font-size: 8pt; font-weight: 700; letter-spacing: .1em;
    color: #888; text-transform: uppercase; border-bottom: 1px solid #ccc;
    display: block; padding-bottom: 3px; }

  /* riga articolo */
  .item-row td { border-bottom: 1px solid #e8e8e8; vertical-align: top; }

  /* colonna quantità — numero grande a sinistra, come nel PDF */
  .col-qty {
    width: 54px; min-width: 54px;
    font-size: 26pt; font-weight: 900; color: #111;
    text-align: right; padding: 10px 14px 10px 0;
    line-height: 1;
    vertical-align: middle;
  }

  /* colonna nome */
  .col-name { padding: 10px 0 10px 6px; vertical-align: middle; }
  .item-name {
    font-size: 13pt; font-weight: 700; color: #111;
    display: block; line-height: 1.25;
  }
  .item-note {
    font-size: 9pt; color: #c06000; font-style: italic;
    display: block; margin-top: 3px; font-weight: 700;
    letter-spacing: .02em;
  }
  .tag-extra { font-size: 8pt; color: #c8a45a; font-weight: 700; }

  /* piè pagina */
  .page-foot { font-size: 8pt; color: #bbb; margin-top: 20px; text-align: right; }
...
<body>
  <div class="page-header">
    <div class="page-title">Hotel Apogeo — Ordine Colazioni</div>
    <div class="page-date">${now}</div>
    <div class="page-meta">${selectedItems.length} articoli · ${selectedItems.reduce((s,i)=>s+i.qty,0)} pezzi totali</div>
  </div>
  ...
  <div class="page-foot">Generato da Ordine Colazioni WebApp</div>
  ...
  else   { alert('Popup bloccato dal browser. Permettere i popup per questa pagina.'); }
});
